...
class PuzzleSolver(object):
    ...
    def calculate_total_cost(self, state):
        ...
        if self.heuristic != 'naive':
            ...
            goal1_idx = state.goal1.index(0)
            goal1_row = goal1_idx // state.m
            goal1_col = goal1_idx % state.m
            if goal1_row == 0 and goal1_col == 0:
                result.append(0)
            else:
                result.append(1)
            if self.algorithm == 'ucs':
    ...
    def printSolutionFile(self, state_list, cost_of_path, running_time):
        if self.algorithm == 'ucs':
            name = str(self.counter) + '_' + self.algorithm + '_solution.txt'
        else:
            name = str(self.counter) + '_' + self.algorithm + '_heuristic_' + str(self.heur) + '_solution.txt'
    ...
    def printSearchFile(self, explored):
        if self.algorithm == 'ucs':
            name = str(self.counter) + '_' + self.algorithm + '_search.txt'
        else:
            name = str(self.counter) + '_' + self.algorithm + '_heuristic_' + str(self.heur) + '_search.txt'
    ...
    def printAnalysis(self, search_depth, nodes_expanded, cost_of_path, running_time):
        if self.algorithm == 'ucs':
            name = self.algorithm + '_analysis.txt'
        else:
            name = self.algorithm + '_heuristic_' + str(self.heur) + '_analysis.txt'
